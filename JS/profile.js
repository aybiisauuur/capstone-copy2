...
if (isProfilePage) {
    ...
    fileInput.addEventListener('change', async function () {
        ...
        const user = auth.currentUser;
        if (!user) {
            showModal("Please log in to upload a profile picture");
            return;
        }
        ...
        try {
            ...
            if (base64Image.length > 750000) {
                showModal("Image is too large. Please choose a smaller image or resize it.");
                return;
            }
            ...
            showModal("Failed to upload image: " + error.message);
        }
    });
